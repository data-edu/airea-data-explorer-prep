...
sff <- sf::read_sf("prep/CommutingZones2020_County_GIS_files/county20.shp") |>
  st_drop_geometry() |>
  as_tibble() |>
  filter(!stringr::str_starts(as.character(GEOID), "02")) |>
  transmute(
...
# thresholds (same semantics as your original)
ratio_thresh <- 2      # top city must be >= 2x #2 to stand alone
share_thresh <- 0.8    # others included if >= 80% of top
...
# --- Ensure every CZ gets a non-missing label (fallback) ---
all_cz <- sff |>
  distinct(CZ)
...
cz_labels <- all_cz |>
  ...
  mutate(
    cz_label = if_else(is.na(cz_label) | cz_label == "", paste0("CZ ", CZ), cz_label)
  )

# --- Final dedup guard: if any labels still duplicate, suffix CZ to make unique ---
cz_labels <- cz_labels |>
  group_by(cz_label) |>
  mutate(n_dup = n()) |>
  ungroup() |>
